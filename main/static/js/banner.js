function bannerStarClient(id, text) {
  if (document.getElementById('bannerStarClient')) {
    document.getElementById('bannerStarClient').remove()
  }

  const bannerStarClient = document.createElement("div");
  bannerStarClient.id = "bannerStarClient";
  bannerStarClient.innerHTML = `
      <button class="close-button" onclick="document.getElementById('bannerStarClient').remove()">&times;</button>
      <div>Оцените ${text}</div>
      <div class="stars-e">
        ${[1, 2, 3, 4, 5]
          .map(
            (i) => `
          <svg class="star" data-value="${i}" width="36" height="36" viewBox="0 0 26 26" xmlns="http://www.w3.org/2000/svg">
  ...
  document.body.appendChild(bannerStarClient);

  const stars = bannerStarClient.querySelectorAll(".star");
  let selectedStars = 4;
  ...
  bannerStarClient
    ...
    .addEventListener("click", () => {
      setStars(id, selectedStars);
      document.getElementById("bannerStarClient").remove();
    });
}

function setStars(id, stars) {
  $.ajax({ type: "GET", url: "/setStars/", data: { id: id, stars: stars } });
}

export { bannerStarClient, setStars };
